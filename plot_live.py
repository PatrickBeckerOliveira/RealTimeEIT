...
def create_live_plot(
    ...
    if plot_image==[]:  # creating image for the first time

        plt.ion()  # allowing dynamic plot
        fig, ax = plt.subplots()
        ...
        tripcolor_keys_map = {"vmin": 0, "vmax": 1}
        tripcolor_kwargs = {k: v for k, v in tripcolor_keys_map.items() if v is not None}
        plot_image = ax.tripcolor(x, y, elements, eit_image, **tripcolor_kwargs, animated=True) # creates the triangular grid plot and enables animated
    ...
    plot_image.set_array(eit_image) # only updates the array of the triangles colors
    ax.draw_artist(plot_image)
    fig.canvas.blit(fig.bbox) 
    fig.canvas.flush_events() 
    return plot_image, ax, fig, bg
